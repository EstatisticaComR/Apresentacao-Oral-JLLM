...
```{r, eval=F}
#Separando por ano
dAnos <- dados %>% count(ano) %>% filter(ano!=2013)
ano <- dAnos %>% 
  pull(ano) %>% 
  as.factor()
freq_ano <- dAnos %>% pull(n) #pull seleciona coluna e retorna como vetor

data.frame(ano, freq_ano, perc=round(100*freq/sum(freq_ano),1))
p <- ggplot(data=data.frame(ano,freq_ano),aes(x=ano, y=freq_ano, fill=as.factor(freq_ano)))+
...
dados_sem <- na.omit(dados %>% count(dia.semana))

dia=c("1.seg","2.ter", "3.qua", "4.qui", "5.sex", "6.sab","7.dom")
freq_dias=c(dados_sem %>% pull(n))
data.frame(dia,freq_dias,perc=round(100*freq_dias/sum(freq_dias),1))

p1=ggplot(data=data.frame(dia,freq_dias),aes(x=dia, y=freq_dias, fill=as.factor(freq_dias)))+
  geom_bar(stat="identity", width=0.5,position="dodge")+
